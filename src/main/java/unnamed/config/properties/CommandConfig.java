package unnamed.config.properties;

import static unnamed.utils.CommandUtils.*;
...
public class CommandConfig extends SidedCommand {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        if (args.length < 2) throw error("unnamed.command.no_enough_args");

        final String command = args[0];
        if (!SUBCOMMANDS.contains(command)) throw error("unnamed.command.invalid_command", command);

        final String modId = args[1];
        final ModConfig config = ConfigProcessing.getConfig(modId);
        if (config == null) throw error("unnamed.command.unknown_modid");

        if (COMMAND_SAVE.equals(command)) {
            config.save();
            respond(sender, "unnamed.command.saved", config.getConfigFile().getAbsolutePath());
            return;
        }

        if (args.length < 4) throw error("unnamed.command.no_enough_args");

        final String category = args[2];
        final String name = args[3];

        ConfigPropertyMeta property = config.getValue(category, name);
        if (property == null) throw error("unnamed.command.unknown_value");
        ...
        if (COMMAND_HELP.equals(command)) {
            ...
            else throw error("unnamed.command.not_multiple");
            return;
        ...
        if (args.length < 5) throw error("unnamed.command.no_enough_args");
        ...
        if (!property.acceptsMultipleValues()) throw error("unnamed.command.not_multiple");
    ...
    protected void changeValue(ModConfig config, ICommandSender sender, ConfigPropertyMeta property, String... values) {
        try {
            ...
            switch (changeResult) {
                case ONLINE:
                    respond(sender, "unnamed.command.online_change");
                    break;
                case OFFLINE:
                    respond(sender, "unnamed.command.offline_change");
                    break;
                default:
                    respond(sender, "unnamed.command.cancelled");
                    break;
            }
            printValue(sender, config, property);
        } catch (StringConversionException e) {
            throw error("unnamed.command.invalid_type", Arrays.toString(values), property.type);
        } catch (Exception e) {
            Log.warn(e, "Error during command change");
            throw error("unnamed.command.unknown_error", e.getMessage());
        }
    }
