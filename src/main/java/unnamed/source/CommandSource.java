package unnamed.source;

import static unnamed.utils.CommandUtils.filterPrefixes;
...
import unnamed.Log;
import unnamed.source.ClassSourceCollector.ApiInfo;
import unnamed.source.ClassSourceCollector.ClassMeta;
...
public class CommandSource extends SidedCommand {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (subCommand.equals(COMMAND_CLASS)) {
            ...
            sender.addChatMessage(new ChatComponentTranslation("unnamed.command.class_source", meta.cls.getName(), meta.source()));

            final ApiInfo api = meta.api;
            if (api != null) {
                sender.addChatMessage(new ChatComponentTranslation("unnamed.command.api_class",
                        api.api, api.owner, api.version));
            }

            for (Map.Entry<File, Set<String>> e : meta.providerMods.entrySet())
                sender.addChatMessage(new ChatComponentTranslation("unnamed.command.class_provider",
    ...
    private ClassMeta getMeta(final String clsName) {
        try {
            ...
            throw new CommandException("unnamed.command.invalid_class_name", clsName);
        } catch (Throwable t) {
            Log.warn(t, "Failed to get information for class %s", clsName);
            throw new CommandException("unnamed.command.unknown_error_details");
        }
    }
